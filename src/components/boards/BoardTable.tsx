...
export default function BoardTable({ posts, totalCount }: {posts : postType[], totalCount : number }) {
  const [pageNum, setPageNum] = useState<number[]>([]);// 페이지 번호 상태
  const [totalPageNum, setTotalPageNum] = useState<number>(0);// 총 페이지 수
  ...
  useEffect(() => {
    ...
    const pages = [];
    for (let i = Math.floor(currentPage / 5) * 5 + 1; i <= Math.floor(currentPage / 5 + 1) * 5; i++) {
      if(i<= totalPageNum) {
        pages.push(i);
      }
    }
          ...
          <li className='py-[12px] border-b border-[#E4E5F0]'>
              <dl className="flex relative flex-wrap">
                ...
                <dd className="w-full md:w-[45%] block">
                  <Link key={post.writer.id} href={`/posts/${post.writer.id}`}>
                    {post.title}
                  </Link>
                </dd>
                ...
                <dd className="w-[15%] ml-[16px] text-[#8F95B2] md:text-[#474D66]">{post.likeCount}</dd>
              </dl>
        </li>
        ))}
        </ul>

        <nav aria-label="Pagination" className="flex -space-x-px rounded-md shadow-xs gap-[11px] justify-center">
            <button type="button"
